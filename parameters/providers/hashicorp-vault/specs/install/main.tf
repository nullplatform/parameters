...
module "hashicorp_vault_configuration" {
  ...
  nrn                         = each.value.nrn
  np_api_key                  = var.np_api_key
  provider_specification_slug = module.hashicorp_vault_spec.slug
  dimensions                  = each.value.dimensions
  attributes                  = local.instance_attributes[each.key]

  depends_on = [module.hashicorp_vault_spec]
}
...
module "hashicorp_vault_channels" {
  ...
  nrn            = each.value.nrn
  api_key        = module.hashicorp_vault_api_keys[each.key].api_key
